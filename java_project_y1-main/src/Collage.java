...
public class Collage implements Serializable {
    ...
    // delegates creation to Degree enum — adding a new degree requires no change here (OCP)
    private Lecturer createLecturer(String name, String id, int degree, String degName,
                                    double salary, int articleSize, String academy, HashSet<String> artSet) {
        return Degree.degFromInt(degree).createLecturer(name, id, degName, salary, articleSize, academy, artSet);
    }
    ...
    public void lecturerToCollage(String name, String id, int degree, String degName, double salary, String depName, int articleSize, String academy, HashSet<String> artSet) {
        Lecturer lecturer = createLecturer(name, id, degree, degName, salary, articleSize, academy, artSet);
    ...
    // delegates creation to Committee factory — Collage no longer knows committee subtypes (SRP/OCP)
    private Committee createCommittee(String comName, Lecturer headLec, int lecLevel) {
        return Committee.createByLevel(comName, headLec, lecLevel);
    }
    ...
    public void committeeToCollage(String comName, String lecName, int lecLevel) throws CollageException {
        ...
        Committee committee = createCommittee(comName, headLec, lecLevel);
    ...
    public void lecturerToCommittee(String com, String lecName) throws CollageException {
        ...
        if (updateCom != null && lecToAdd != null) {
            if (updateCom.getHeadOfCommittee().equals(lecToAdd)) {
                ...
                throw new AlreadyMemberException();
            } else if (updateCom.getType().isAssignableFrom(lecToAdd.getClass())) {
    ...
    public String showAllLecturers() {
        ...
        while (it.hasNext()) {
            Lecturer lec = it.next();
            if (lec != null) {
                res.append(lec.toString()).append("\n");
            }
        }
        return res.toString();
    }
    ...
    public Lecturer compareLec(String lecName1, String lecName2) throws CollageException {
        ...
        if (!lecturer1.isQualifiedForHead() || !lecturer2.isQualifiedForHead()) {
            throw new NotProfDocException();
        }
    ...
    public String comByNumOfArt(String firstCom, String secCom, boolean opt) throws CollageException {
        ...
        java.util.Comparator<Committee> comparator = opt ? new SortComByArc() : new SortComByNumOfLec();
        int res = comparator.compare(com1, com2);
        if (res == 0) {
    ...
    public void cloneCom(String com) throws CollageException {
        ...
        try {
            ...
            while (it.hasNext()) {
                ((Lecturer) it.next()).addCom(newCom);
            }
